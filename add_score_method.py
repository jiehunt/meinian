...
for i in target:
    train_ml[i]=train_ml[i].apply(lambda x:log1p(x))
...
for classname in target:
    ...
    ml_predictor.train(train_ml[cols],model_names=['LGBMRegressor'],scoring='brier_score_loss')#,training_params=params[class_name])#,feature_learning=True, fl_data=sam,verbose=False
...
print('****** over to train ')
for i in target:
    score[i]=score[i].apply(lambda x:expm1(x))    
