...
get_ipython().system('pip install --user lightgbm')
...
# In[2]:
#数据集已经上传到git
data=pd.read_csv('cleaned_data_0418.csv',low_memory=False,encoding='gbk')
...
lgb_param_s = {
        ...
        'metric' : {'error'},
        'num_leaves' : 31*2, 'learning_rate' : 0.01, 'feature_fraction' : 0.9,
...
lgb_param_h = {
        'task' : 'train', 'boosting_type' : 'gbdt', 'objective' : 'regression',
        'metric' : {'error'},
        'num_leaves' : 31*2, 'learning_rate' : 0.3, 'feature_fraction' : 0.9,
        'bagging_fraction' : 0.8, 'bagging_freq': 5, 'verbose': 1
       # 'scale_pos_weight':40., # because training data is extremely unbalanced
}
lgb_param_l = {
